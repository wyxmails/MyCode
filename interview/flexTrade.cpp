...
int main(int argc, char* argv[])
{
	...
	cout << "sssssssssssss" << i << endl;	
	cout << Stocks[i-1] << " " << Intervals[i-1] << " " << Volumes[i-1] << endl;
		cerr << Stocks[i] << endl;
	...
	cerr << "Writing files a" << endl;
	
	// write file a
	for (int s = 0; s <= i; ++s)
	{
		cout << Stocks[s] << "," << Intervals[s] << ",";
		//cout << TotalVolumes[Stocks[s]] << "," << Volumes[s] << endl;
		if(Volumes[s])
			cout << (double)Volumes[s]/TotalVolumes[Stocks[s]] << endl;
		else
			cout << 0 << endl;
	}
	
	cerr << "Writing files b" << endl;
	// write file b
