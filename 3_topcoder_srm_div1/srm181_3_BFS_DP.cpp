...
struct Q{
	int shot,his;
	Q(int s,int his):shot(s),his(his){}
...
class KiloManX{
public:
	int leastShots(vector <string> dam, vector <int> bh){
		...
		while(!mq.empty()){
			Q cur = mq.top();
			mq.pop();
			if(visited[cur.his]) continue;
			visited[cur.his] = true;
			//cout << cur.num << " " << cur.his << " " << cur.shot << endl;
			if(cur.his==des) return cur.shot;
			for(int i=0;i<n;++i){
				if((1<<i)&cur.his) continue;
				int best = bh[i];
				for(int w=0;w<n;++w){
					if(dam[w][i]=='0'||w==i) continue;
					if((1<<w)&cur.his){
						int step = bh[i]/(dam[w][i]-'0');
						if(bh[i]%(dam[w][i]-'0')) step++;
						best = min(best,step);
					}
				}
				Q next(cur.shot+best,cur.his|(1<<i));
				mq.push(next);
			}
		}
		return -1;
